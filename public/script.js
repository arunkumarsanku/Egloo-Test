...
const escapeHtml = (s) => // Escape HTML special characters
 String(s)
    ...
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#039;");
function renderIcons(icons, duration) {
    return `
    <div class="icons">
      ${icons.map((name) => {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (cardsContainer)
        cardsContainer.innerHTML = CARDS.map(renderCard).join("");
    if (menu)
        menu.innerHTML = CARDS.map(renderMenuItem).join("");
});
